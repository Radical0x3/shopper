...
function mobileToggle() {
  if ($(window).width() < 769) {
    ...
    $("#catalog .header__list-link").on("touchend", function (e) {
      e.preventDefault();
...
search.on("focus", function () {
  let elem = $(this).parent().find(".form__result");

  if ($(this).val() !== "" && $(this).val() !== "test") {
    elem.addClass("active");
    elem.find(".form__result-fail").removeClass("active");
    elem.find(".form__result-success").addClass("active");
  } else if ($(this).val() !== "" && $(this).val() === "test") {
    elem.addClass("active");
    elem.find(".form__result-success").removeClass("active");
    elem.find(".form__result-fail").addClass("active");
  } else {
    elem.removeClass("active");
  }
});
...
let container = $("#header-form");
$(document).mouseup(function (e) {
  let elem = $(".form__result");
  if (container.has(e.target).length === 0 && elem.has(e.target).length === 0) {
    elem.removeClass("active");
  }
});
...
changeCart();
$(".user__cart-remove").on("click", function (e) {
  e.preventDefault();
  $(this).parents(".user__cart-item").remove();
  changeCart();

  let cart = $(".basket span");
  let val = parseInt(cart.text());
  cart.text(val - 1);
});

function changeCart() {
  let title = $(".user__cart-title span");
  let count = $(".user__cart-item").length;
  let body = $(".user__cart-body");
  let empty = $(".user__cart-empty");
